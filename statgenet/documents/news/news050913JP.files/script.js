...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "news050913JP.files";
var g_showoutline = 1;
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			/*if ( IsFramesMode() )
				StatusPlay("このプレゼンテーションを表示するには、お使いのブラウザの旧バージョンが適しています。現在お使いの Microsoft Internet Explorer あるいは Netscape Navigator のブラウザの機能を活用するには、このプレゼンテーションを最適化する必要があります。"); */
			jpegArray( 1 );
		}
		else
			window.location.replace(  "news050913JP.files/error.htm" );
	}
	else {
	...
	if ( IsFramesMode() && !isWebTV() )
		StatusPlay("このプレゼンテーションに含まれる一部コンテンツは、お使いのブラウザで正しく表示されない場合があります。Microsoft Internet Explorer または Netscape Navigator のより最近のバージョンに適しています。");
	*/
	if ( isFLATFiles <= 0 ) {
	/*	if ( IsFramesMode() )
			window.alert("このプレゼンテーションに含まれる一部コンテンツは、お使いのブラウザで正しく表示されない場合があります。Microsoft Internet Explorer または Netscape Navigator のより最近のバージョンに適しています。" );
	*/
		window.location.replace(  "news050913JP.files/error.htm" );
	}		
	/*
